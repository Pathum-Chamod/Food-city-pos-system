...
class PresentationCashierSummaryScreen extends StatefulWidget {
  const PresentationCashierSummaryScreen({super.key});

  @override
  State<PresentationCashierSummaryScreen> createState() =>
      _PresentationCashierSummaryScreenState();
}

enum PresentationSummaryRange { today, last7Days, last30Days, specificDate }

class _PresentationCashierSummaryScreenState
    extends State<PresentationCashierSummaryScreen> {
  ...
  PresentationSummaryRange _selectedRange = PresentationSummaryRange.today;
  DateTime? _selectedDate;
  PresentationCashierSummary? _summary;
  ...
  ({DateTime start, DateTime end}) _currentRange() {
    ...
    if (_selectedRange == PresentationSummaryRange.specificDate &&
        _selectedDate != null) {
  ...
  Future<void> _loadSummary() async {
    ...
    try {
      ...
      final sessionStartedAt = auth.presentationSessionStartedAt;

      final summary = await PresentationModeService.instance.getCashierSummary(
        start: range.start,
        end: range.end,
      ...
      AppSnackBar.show(
        context,
        message: 'Could not load cashier summary: $e',
  ...
  Future<void> _pickSpecificDate() async {
    ...
    setState(() {
      ...
      _selectedRange = PresentationSummaryRange.specificDate;
    });
    _loadSummary();
  }
  ...
  String _formatRangeText() {
    ...
    if (_selectedRange == PresentationSummaryRange.today) {
  ...
  Widget _buildRangeChip(PresentationSummaryRange value, String label) {
    ...
    return ChoiceChip(
      ...
      onSelected: (_) {
        ...
        setState(() {
          ...
          if (value != PresentationSummaryRange.specificDate) {
            _selectedDate = null;
          }
        });
        _loadSummary();
  ...
  Widget _buildNetSalesHero(PresentationCashierSummary? summary) {
    final netSales = summary?.netSales ?? 0;
    final transactions = summary?.transactionCount ?? 0;
    final itemsSold = (summary?.itemsSold ?? 0).toDouble();
    final refunds = summary?.refundTotal ?? 0;
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: _withMetaDividers([
                    ...
                    _buildMetaText(
                      '${_formatQuantity(itemsSold)} Item${itemsSold == 1 ? '' : 's'} sold',
                    ),
  ...
  Widget _buildMetricsGrid(PresentationCashierSummary? summary) {
    final transactions = summary?.transactionCount ?? 0;
    final itemsSold = summary?.itemsSold ?? 0;
    final avgSale = summary?.averageSale ?? 0;
    final cashSales = summary?.cashSales ?? 0;
    final cardSales = summary?.cardSales ?? 0;
    final discounts = summary?.totalDiscounts ?? 0;
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final items = [
          ...
          _buildMetricTile(
            label: 'Items Sold',
            value: _formatQuantity(itemsSold),
