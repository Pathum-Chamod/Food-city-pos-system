...
void main() {
  ...
  test('changing price type only affects newly added cart lines', () async {
    ...
    await cart.setPriceType(ProductPriceType.wholesale);
    expect(cart.items[0].priceType, ProductPriceType.selling);
    expect(cart.items[0].unitPrice, 100);
    ...
    await cart.setPriceType(ProductPriceType.sale);
    await cart.addToCart(product);
  ...
  test('existing cart lines can still be intentionally repriced', () async {
    final cart = CartProvider();
    final product = testProduct();

    await cart.addToCart(product, quantity: 2);
    await cart.setPriceType(
